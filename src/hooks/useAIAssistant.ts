  ...
  const sendMessage = useCallback(async (input: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try { errorData = errorText ? JSON.parse(errorText) : {}; } catch { errorData = { message: errorText }; }
      ...
      const consumeSseLine = (line: string) => {
        ...
        try {
          ...
          // Keep malformed partial chunks in the buffer path instead of killing the answer.
        }
      };
